...
export default function Dashboard() {
  ...
  const fetchStats = async () => {
    try {
      setLoading(true);
      setError('');
      console.log('📡 Fetching dashboard stats for role:', user?.role);
      const response = await API.get('/dashboard/stats');
      console.log('✅ Stats received:', {
        role: response.data.role,
        totalForms: response.data.totalForms,
        totalSubmissions: response.data.totalSubmissions,
        recentSubmissions: response.data.recentSubmissions?.length
      });
      ...
      <nav className="bg-white shadow-sm px-6 py-4">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-gray-500">Logged in as: {user.role} ({user.email})</p>
          </div>
